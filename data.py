...
def get_pages_count(driver):
    return  int(driver.find_element(by=By.CLASS_NAME,value="total-pages").text)
...
def add_course_num(driver,row_el):
...
def process_table_cell(cell,index,driver,row_el):
    if index == 0:
        open_section_details(cell)
        add_course_num(driver,row_el)
...
def extractTableRow(row,table_cells,driver):
    ...
    index = 0
    for cell in cells:
        process_table_cell(cell,index,driver,row_el)
        index += 1
